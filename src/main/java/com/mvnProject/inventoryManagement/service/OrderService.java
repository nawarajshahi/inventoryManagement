...
import com.mvnProject.inventoryManagement.repository.OrderRepository;
import com.mvnProject.inventoryManagement.repository.ProductRepository;
import com.mvnProject.inventoryManagement.util.MembershipLevel;
import com.mvnProject.inventoryManagement.util.OrderStatus;
...
public class OrderService {
	...
	public Orders submitNewOrder(Set<Long> productIds, Long customerId) throws Exception{
		try {
			...
			logger.error("Exception occvured while trying to create new order for customer: " + customerId, e);
			throw e;
		}
	}
	...
	public Orders completeOrderr(Long orderId) throws Exception{
		try {
			Orders order = repo.findOne(orderId);
			order.setStatus(OrderStatus.DELIVERED);
			return repo.save(order);
		} catch (Exception e) {
			logger.error("Exception occured while trying to complete the order: " + orderId, e);
			throw new Exception("Unable to complete the order");
		}
	}
	...
	private Orders initializeNewOrder(Set<Long> productIds, Customer customer) {
		...
		order.setProducts(convertToProductSet(productRepo.findAll()));
